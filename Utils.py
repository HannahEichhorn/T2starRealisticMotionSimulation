...
def load_all_echoes(in_folder, descr, nr_echoes=12):
    '''
    ...
    dataset = np.zeros((nr_echoes, *shape), dtype=np.complex)

    for re, im, i in zip(files_real, files_imag, range(0, nr_echoes)):
        dataset[i] = np.rollaxis(nib.load(files_real[i]).get_fdata(), 2, 0) + \
                     1j * np.rollaxis(nib.load(files_imag[i]).get_fdata(), 2, 0)
...
def apply_transform_image(image, par, pixel_spacing=[6.3, 2, 2]):
    '''
...
def transform_sphere(dset_shape, motion_parameters, pixel_spacing, radius):
    '''For given motion parameters, transform a sphere with certain radius'''
...
def augment_motion_parameters(T, R):
    augm = np.random.randint(0, 3)
    if augm == 0:
        # rescale globally:
        augm_T = np.random.uniform(1, 3)
        augm_R = np.random.uniform(1, 2)

        T = T * augm_T
        R = R * augm_R

    augm = np.random.randint(0, 2)
    if augm == 0:
        # rescale locally:
        t = np.random.uniform(1, 4, [5])
        r = np.random.uniform(1, 3, [5])

        points = sorted(np.random.randint(1, len(T) - 2, 4))
        augm_T = np.ones([len(T)])
        augm_T[0:points[0]] = t[0]
        augm_T[points[0]:points[1]] = t[1]
        augm_T[points[1]:points[2]] = t[2]
        augm_T[points[2]:points[3]] = t[3]
        augm_T[points[3]:] = t[4]
        augm_R = np.ones([len(T)])
        augm_R[0:points[0]] = r[0]
        augm_R[points[0]:points[1]] = r[1]
        augm_R[points[1]:points[2]] = r[2]
        augm_R[points[2]:points[3]] = r[3]
        augm_R[points[3]:] = r[4]

        T = T * np.array([augm_T, augm_T, augm_T]).T
        R = R * np.array([augm_R, augm_R, augm_R]).T
    ...
    if augm == 0:
        ...
        augm_T = np.arange(a, len(T)) * t / len(T)
        augm_R = np.arange(a, len(T)) * r / len(T)

        T[a:] += np.array([augm_T, augm_T, augm_T]).T
        R[a:] += np.array([augm_R, augm_R, augm_R]).T

    return T, R


class SimulateMotionForEachReadout:
    def __init__(self, motion_tracking, nr_pe_steps, brainmask, include_transform=True, include_inhomog=False,
                 path_scan_order="/home/iml/hannah.eichhorn/Code/Motion_simulation/Scan_order.txt",
    ...
    def create_random_B0_inhom(self, dataset, times):
        ...
        # perform dilation / erosion on brainmask to ensure no holes:
        ...
        for t in range(1, int(times[-1]/10)+1):
            ...
            x_mesh, y_mesh = np.asarray(np.meshgrid(*ranges))
    ...
    def simulate_all(self, dataset, radius_sph=64):
        '''
        Simulate motion for all readout lines and combine them
        '''
        max_slices = dataset.shape[1]
...
class ImportMotionDatafMRI:
    def __init__(self, filename, scan_length, reference_to_0=True, random_start_time=True):
        super().__init__()
        self.reference_to_0 = reference_to_0
        self.motion_data = np.loadtxt(filename)
        TRs = {'_dsc': 1.3, '-fMRI': 1.5, '_CVR': 1.2}
        for t in TRs:
            if t in filename:
                TR = TRs[t]

        self.seconds = np.arange(0, len(self.motion_data)*TR, TR)

        # concatenate too short motion curves:
        while self.seconds[-1] < scan_length:
            self.motion_data = np.append(self.motion_data, self.motion_data, axis=0)
            self.seconds = np.append(self.seconds, self.seconds + self.seconds[-1], axis=0)
        ...
        if random_start_time:
            ...
            ind = np.intersect1d(np.where(self.seconds >= random_start), np.where(self.seconds <= (random_start+scan_length)))
    ...
    def get_motion_data(self, dset_shape, pixel_spacing=[3.3, 2, 2], radius=64, augment=False):
        """
        Get motion parameters from transformation matrices. If self.reference_to_0 is set to True,
        the transformation parameters are first transformed so that the reference (median) position corresponds to
        ...
        T, R = self.motion_data[:, :3], self.motion_data[:, 3:]*180/np.pi # convert radians to degrees

        if augment:
            T, R = augment_motion_parameters(T, R)
        ...
        if not self.reference_to_0:
            ...
            centroids, tr_coords = transform_sphere(dset_shape, np.append(T, R, axis=1),
